...
for l = 1:length(lambda)
    for k = 1:numRepeats
        ...
        time_lsc(k) = etime(endTime, startTime);        
            ...
            if min(num_clus)==0
                ...
                Nentro(k) = -1/log(numClasses)* sum(nk.*log(nk));
            end
            counts = num_clus;
    ...
    lsctim = mean(time_lsc);
       mer=mean(result);
        ...
        fprintf(fid, 'time = %0.6f ACC = %0.2f(%0.2f) NMI = %0.2f(%0.2f) Purity = %0.2f(%0.2f) P = %0.2f(%0.2f) R = %0.2f(%0.2f) F = %0.2f(%0.2f) RI = %0.2f(%0.2f) NE = %0.2f(%0.2f) SDCS = %0.2f(%0.2f)\r\n',lsctim,mer(1),sttd(1),mer(2),sttd(2),mer(3),sttd(3),mer(4),sttd(4),mer(5),sttd(5),mer(6),sttd(6),mer(7),sttd(7),mNe,stdNe,mSD,stdSD);
